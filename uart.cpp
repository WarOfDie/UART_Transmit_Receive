...
UARTBase::UARTBase(UART_HandleTypedef_t *USART_Handle, RSXXX_ModeTypeDef_t RSXXX)
{
	init(USART_Handle, RSXXX);
}
/**
 ...
 * @retval void
 */
void UARTBase::init(UART_HandleTypedef_t *USART_Handle ,RSXXX_ModeTypeDef_t RSXXX )
{
	uint32_t tempReg = 0;
	...
	if(mode == RS485)
	{
		//HAL_GPIO_Init(RS485_CTRL_PORT, GPIO)
	}
}
/**
...
/**
 * @brief  RS485 DE ve RE pinlerini kontrol eder.
 * @param  bool  true ya da false değer alır.
 * @retval void
 */
void UARTBase::set_rs485_control(bool enable) {
	if (mode == RS485) {
		HAL_GPIO_WritePin(RS485_CTRL_PORT, RS485_CTRL_PIN, enable ? GPIO_PIN_SET : GPIO_PIN_RESET);
	}
}
